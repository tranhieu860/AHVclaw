...
func ListConversations(c *fiber.Ctx) error {
	...
	rows, err := db.Pool.Query(context.Background(),
		`SELECT id, user_id, title, model, agent_id, pinned, created_at, updated_at, channel
	...
	for rows.Next() {
		...
		if err := rows.Scan(&conv.ID, &conv.UserID, &conv.Title, &conv.Model,
			&conv.AgentID, &conv.Pinned, &conv.CreatedAt, &conv.UpdatedAt, &conv.Channel); err != nil {
			continue
		}
