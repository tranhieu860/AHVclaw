...
func (a *Adapter) handleCommand(msg *tgbotapi.Message) {
	...
	case "trust":
		...
		entries, _ := autonomous.ListTrust(ctx, userID)
		if len(entries) == 0 {
			a.sendText(chatID, "Chưa có dữ liệu trust. Hệ thống sẽ học dần qua các hành động.")
			return
		}
		var text strings.Builder
		text.WriteString("🔐 Trust Permissions:\n\n")
		for _, e := range entries {
			text.WriteString(fmt.Sprintf("• %s/%s: %d/10\n", e.ActionType, e.ActionPattern, e.TrustScore))
		}
		a.sendText(chatID, text.String())
	}
}
